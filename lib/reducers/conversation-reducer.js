...
var addMessage = function addMessage(messages, message) {
    ...
    if ((0, _messages.isHiddenMessage)(message)) {
        return messages;
    }
    ...
    switch (action.type) {
        ...
        case ConversationActions.SET_CONVERSATION:
            return (0, _extends3.default)({}, action.conversation, {
                messages: (0, _messages.filterHiddenMessages)(state.messages),
                replyActions: state.replyActions
            });
        case ConversationActions.SET_MESSAGES:
            return (0, _extends3.default)({}, state, {
                messages: assignGroups((0, _messages.filterHiddenMessages)(sortMessages(cleanUpMessages([].concat(action.messages, preserveFailedMessages(state.messages)))))),
                replyActions: extractReplyActions(action.messages[action.messages.length - 1])
            });
        case ConversationActions.ADD_MESSAGES:
            return (0, _extends3.default)({}, state, {
                messages: assignGroups((0, _messages.filterHiddenMessages)(sortMessages(cleanUpMessages(action.append ? [].concat(state.messages, action.messages) : [].concat(action.messages, state.messages))))),
                replyActions: extractReplyActions(action.messages[action.messages.length - 1])
            });
        ...
        case ConversationActions.REPLACE_MESSAGE:
            return (0, _assign2.default)({}, state, {
                messages: assignGroups((0, _messages.filterHiddenMessages)(sortMessages(replaceMessage(state.messages, action.queryProps, action.message))))
            });
